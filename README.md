...
The console is a signed-in clinical workstation, not a dashboard. Your badge and role decide what the board allows: a triage nurse sets acuity levels, a medical assistant records vitals but cannot, an administrator reads without touching. Six sections:

| | |
|---|---|
| **Patient queue** | Ranked by acuity, with each patient's level, belief, wait against the safe limit for that level, and vitals trend |
| **Monitoring** | The same people ranked by reassessment priority, which is a different question and gets its own screen |
| **Pipeline** | The run that produced the selected patient's recommendation, with measured time per stage |
| **Registry** | Every component, with model ids read from live configuration |
| **Analytics** | The held-out benchmark, the bias monitor, and every clinician decision with the badge attached |
| **Settings** | The thresholds this department is actually running |

Alerts sit above all of it, and answering one is two clicks: reassess, or acknowledge. Both are written to the audit trail. Panes drag to whatever width the workstation has, and there is a night theme, because a twelve-hour shift is not always a daytime one.
...
1. **Phase 1, Intake.** Chief complaint in free text, vitals, age, AVPU, medications if on file, and an optional OLDCARTS interview whose severity answer feeds the ESI pain gate. A two-tier classifier reads the complaint: deterministic clinical rules first, and a distilled model only where the rules are silent ([how it was built and why](#the-intake-classifier-distillation-end-to-end)). The category auto-codes to a provisional ICD-10. Half of real patients arrive with no record at all, and the system is designed for that.
...
What ships instead is two layers:

- **A deterministic rule tier**, which is the guaranteed-recall contract: a red-flag phrase maps to its protocol 100% of the time, provably, pinned by tests.
- **A distilled classifier** beneath it, for where the rules are silent. [Model2Vec](https://github.com/MinishLab/model2vec) static embeddings (a full sentence transformer distilled into a 30MB, numpy-only artifact, MIT) with a class-balanced logistic head, trained by `scripts/train_complaint_classifier.py` on clinically reviewed labels over real MIMIC-IV-ED chief complaints plus adversarial phrasings.

Softmax gives bounded, calibrated probabilities, so the model accepts a high-risk call at lower confidence than a benign one and abstains below both floors. Training is zero-init on a convex objective, so the committed artifact is bit-reproducible with no random seed, and the run reports its own loss curve so a converged fit is verified rather than assumed.

Two guards keep the claim honest. A committed snapshot fixture freezes the classification of every benchmark and demo text, making drift a visible, reviewed event. And the disguised-presentation probes in the test suite are measured against every training row, so a future row that paraphrases one of them turns the suite red instead of quietly inflating the result. Fixing the next unseen phrasing is one labeled row in `data/complaint_examples.json` and a deterministic retrain.
