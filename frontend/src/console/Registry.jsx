    ...
    <article className="border border-line rounded-md bg-card">
      <button onClick={() => setOpen(!open)}
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <h3 className="text-[13px] font-bold text-ink">{c.name}</h3>
          ...
          <span className="ml-auto flex items-center gap-3 text-[11px] text-ink-2 tabular-nums">
            <span>{c.invocations} runs</span>
            {c.latency_ms > 0 && <span>{c.latency_ms.toFixed(1)} ms avg</span>}
      ...
      {open && (
        <dl className="px-4 pb-3.5 pt-1 grid sm:grid-cols-3 gap-3 border-t border-line">
...
function Group({ inside, label, components }) {
  ...
  return (
    <Card>
      <CardHead title={
        <span className="flex items-center gap-1.5"><Icon size={12} aria-hidden="true" />{label}</span>
      } />
...
export default function Registry({ refreshKey }) {
  ...
  const inside = data.components.filter((c) => c.boundary === 'phi')
  const outside = data.components.filter((c) => c.boundary === 'deidentified')
      ...
      <Card>
        <div className="px-4 py-3.5">
          ...
          <p className="mt-1 text-[11.5px] leading-relaxed text-ink-2 max-w-3xl">
            Eight components produce every recommendation. Model identifiers and
            thresholds below are read from the running configuration, not restated,
            so this page cannot drift from the system it describes. The split is
            the privacy boundary, and exactly one component ever sends anything
            off this machine: the clinical reasoning path, which has only seen a
            de-identified copy.
          </p>
        </div>
      </Card>

      <Group inside label={data.boundary.phi} components={inside} />
      <Group label={data.boundary.deidentified} components={outside} />
    </div>
  )
}
